...
- **`DayNavigator`** is a static utility class (`DayNavigator.format(DateTime)`) that returns a formatted date string ("Today", "Yesterday", "Tomorrow", or "Jan 15, 2026"). The chevron buttons live in `AppBar.leading`/`actions` in `CombinedLogScreen`, while only the date `Text` is the `title` with `centerTitle: true` — this makes the date text truly screen-centered.
