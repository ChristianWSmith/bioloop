...
- `foods` table has 12 columns: id, name, servingLabel, servingQuantity (default 1.0), servingUnit (default 'serving'), caloriesPerServing, proteinPerServing, carbsPerServing, fatPerServing, barcode (nullable, unique), source (default 'manual'), createdAt
...
- **Search delegate recent foods**: `FoodSearchDelegate` shows `_RecentFoodsSection` (ConsumerWidget watching `recentFoodsProvider`) when query is empty; tapping selects the food as if it were a search result; trailing "+" icon calls `onQuickLog` which opens `QuickFoodLogSheet`
...
- Schema v1→v2 migration (onUpgrade) adds `serving_unit` and `serving_quantity` columns to `foods` and backfills from `serving_label` / `serving_size_grams` patterns; schema v2→v3 drops the vestigial `serving_size_grams` column
