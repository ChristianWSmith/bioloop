...
- `test/features/logging/search_delegate_test.dart` tests `FoodSearchDelegate` toggle interaction (opening, toggling segments, typing + Enter), deletion refresh (food disappears from list immediately after delete), brand display in local and web ListTiles, and web search tap opening `ManualFoodForm` pre-filled (save inserts to DB, delegate stays open; cancel does not save)
...
- `getRecentFoods()` / `searchLocalByRecency()` in `AppDatabase`: `searchLocalByRecency()` partitions foods into 3 groups: (A) OFF-imported foods never logged, sorted by `createdAt DESC`; (B) all logged foods, sorted by last `loggedAt DESC`; (C) never-logged non-imported foods, sorted alphabetically. Concatenates A + B + C, then filters by query and takes limit. `getRecentFoods()` fetches distinct `foodId`s from `food_entries` in a Dart-side loop (drift 2.31.0 has no `groupBy` on `SimpleSelectStatement`); orders by `MAX(loggedAt)` DESC, limit 10, then fetches `Food` records from the `foods` table
