...
3. After confirming:
   - `resetAll()` truncates all 4 tables (`foods`, `food_entries`, `bodyweight_entries`, `user_goals`)
   - Navigate back to onboarding flow (same as fresh install — T2b)

### What is preserved
- App theme settings (system-level, not app-managed)
- Nothing else — all app data is in the 4 drift tables
...
- All 4 tables are empty after reset (verify via DB inspector)
...
- **Unit — `resetAll()` truncates tables**: insert data into all 4 tables, call `resetAll()`, verify each table has 0 rows
...
- [ ] Check DB contents (via debug/inspector) — all 4 tables empty after reset
